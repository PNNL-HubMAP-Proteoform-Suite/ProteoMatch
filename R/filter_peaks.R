...
#'
#' @description For best results in the ProteoMatch algorithm, it's best to filter
#'     by a specific M/Z range and a noise threshold.
#'
#' @param peak_data A peak_data object generated by get_peak_data (from mzML or raw),
#'     or make_peak_data in pspecterlib. Required.
#' @param MZRange A range of MZ values to filter the data by. Default is 300 - 13000.
#'     It is highly recommended that users visualize the spectra first to determine
#'     a reasonable cutoff range.
...
filter_peaks <- function(PeakData,
  ...
  if (!inherits(PeakData, "peak_data")) {
    stop("PeakData should be a peak_data object from the pspecterlib get_peak_data or make_peak_data classes.")
  }
